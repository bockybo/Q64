...
class Renderer: NSObject, MTKViewDelegate {
	...
	private struct Flight {
		let cambuf = util.buffer(len: sizeof(CAM.self), label: "scn")
		let mdlbuf = util.buffer(len: sizeof(MDL.self) * Renderer.maxnmodel, label: "mdl")
		let lgtbuf = util.buffer(len: sizeof(LGT.self) * Renderer.maxnlight, label: "lgt")
	...
	private let quaddepth = util.depthstate {descr in
		descr.frontFaceStencil.stencilCompareFunction		= .equal
		descr.backFaceStencil.stencilCompareFunction		= .equal
	}
//	can't get it to work properly without culling when i'm inside the vol
//	private let maskdepth = util.depthstate {descr in
//		descr.depthCompareFunction							= .lessEqual
//		descr.frontFaceStencil.depthFailureOperation		= .decrementClamp
//		descr.backFaceStencil.depthFailureOperation			= .decrementClamp
//	}
	private let volumedepth = util.depthstate {descr in
		descr.depthCompareFunction							= .greaterEqual
		descr.frontFaceStencil.stencilCompareFunction		= .equal
		descr.backFaceStencil.stencilCompareFunction		= .equal
//		descr.frontFaceStencil.stencilCompareFunction		= .less
//		descr.backFaceStencil.stencilCompareFunction		= .less
	}
	...
	func draw(in view: MTKView) {
		...
		self.cmdque.commit(label: "shade") {buf in
			for i in 0..<self.scene.lighting.count {
				...
				buf.pass(label: "shade \(i)", descr: util.passdescr {descr in
		...
		self.cmdque.commit(label: "light & drawable") {buf in
			...
			buf.pass(label: "light & drawable", descr: util.passdescr {descr in
				...
				enc.setDepthStencilState(self.gbufdepth)
				...
				for model in self.scene.models {
					...
					enc.setFragmentBytes(&defaults, length: sizeof(defaults), index: 4)
				...
				enc.setCullMode(.front)
				enc.draw(mesh: lib.quadmesh, nid: nid)
				...
				enc.draw(mesh: lib.conemesh, iid: iid, nid: nid)
				iid += nid; nid = self.scene.lighting.icos.count
				enc.setRenderPipelineState(self.icospipe)
				enc.draw(mesh: lib.icosmesh, iid: iid, nid: nid)
